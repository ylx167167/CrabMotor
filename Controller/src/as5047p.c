...
AS5047_TypeDef Devs[] = {{&hspi1, GPIOA, GPIO_PIN_4},
                         {&hspi1, GPIOA, GPIO_PIN_3}};

#define assert_idx(__devidx)                                                   \
  if (__devidx < 0 && __devidx > sizeof(Devs) / sizeof(AS5047_TypeDef))        \
    error_inf_loop();
#define CS_1(__devidx)                                                         \
  HAL_GPIO_WritePin(Devs[__devidx].CSNport, Devs[__devidx].CSNpin, GPIO_PIN_SET)
#define CS_0(__devidx)                                                         \
  HAL_GPIO_WritePin(Devs[__devidx].CSNport, Devs[__devidx].CSNpin,             \
                    GPIO_PIN_RESET)
...
void AS5047_SetZeroPosition(int devidx) {
  ...
  uint16_t ANGLEUNC =
      AS5047_ReadData(devidx, ANGLEUNC_AS5047P_VOL_REG_ADD); // 获取当前角度
  //												ANGLEUNC是13:0
  // 14个有效数字，右移6获取高8位。
